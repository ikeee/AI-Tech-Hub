// Moebius DDIM 调度器（移植自 simonw/moebius-web，Apache-2.0）
// scaled_linear beta schedule, eta=0, clip_sample=false
export const NUM_TRAIN_TIMESTEPS = 1000
export const BETA_START = 0.00085
export const BETA_END = 0.012

export interface DDIM {
  alphasCumprod: Float64Array
  /** 降序 timesteps（已按 strength 截断） */
  timesteps: number[]
}
...
export function makeDDIM(numSteps: number, strength = 0.99): DDIM {
  ...
  // strength<1 时丢弃开头 timesteps；Python 用 int() 截断，必须 Math.floor
